...
public class MapGenerator : MonoBehaviour
{
    ...
    private void DestroyEnemiesOutsideMap()
    {
        GameObject[] enemies = GameObject.FindGameObjectsWithTag("Enemy");
        ...
        foreach (GameObject enemy in enemies)
        {
            ...
            // 맵 경계 안에 있는지 확인
            if (enemyPosition.x < -mapSize.x / 2 || enemyPosition.x > mapSize.x / 2 ||
                enemyPosition.y < -mapSize.y / 2 || enemyPosition.y > mapSize.y / 2)
            {
                // 맵 밖에 있는 적을 제거
                Destroy(enemy);
            }
